...
for sitei=7:7
    ...
    for i=1:fn
        ...
        %% Check Melting Layer Presence and Height
                ...
                if nanmask(ti)>10&&max_z>-10
                   [BB_height(ti), BB_bottom(ti), BB(ti), BB_bin] = BB_mask_func_nl(height_nc,z(:,ti),vel(:,ti),1,NaN,NaN);
                end
            end
            for trial=2:6
                for ti=1:num_tim
                    ...
                    if left+right+mid>0&&nanmask(ti)>10&&max_z>-10
                        med_top=median(BB_height(leftbound:rightbound),'omitnan');
                        med_bot=median(BB_bottom(leftbound:rightbound),'omitnan');
                        [BB_height(ti), BB_bottom(ti), BB(ti), BB_bin] = BB_mask_func_nl(height_nc,z(:,ti),vel(:,ti),trial,med_top,med_bot);
                    end
                    if left+right+mid>0&&nanmask(ti)>10&&max_z>-10
                        med_top=median(BB_height(leftbound:rightbound),'omitnan');
                        med_bot=median(BB_bottom(leftbound:rightbound),'omitnan');
...
%                     elseif BB(ti)>0&&sum(BB(max(ti-4,1):min(ti+4,1)))<2&&trial==6
            ...
            for ti=1:num_tim
                ...
                if BB(ti)>0&&sum(BB(leftbound:rightbound))<2
                    BB(ti)=0;
                    BB_height(ti)=NaN;
                    BB_bottom(ti)=NaN;
...
%% If BB detected
        ...
        %% Read METfile
            ...
            filex=dir(matfname);
            if ~isempty(filex)
                save(matfname,'BB_height','BB_all','BB_bottom','BB_lr','BB_T','BB_v','-append')
            else
                save(matfname,'BB_height','BB_all','BB_bottom','BB_lr','BB_T','BB_v')
            end
            save(matfname,'BB','-append')
            clear BB_height BB BB_T BB_lr BB_v
            clear BB BB_height z vel
        end
    end
    Bi=0
    ...
    if ~isempty(filex)
        ...
        save([site 'Cloud_BB_Rain'],'totalnum','cloud_num','BB_num','BB_height_bin');
    end
end
% era5comp
%BB_contour_figs
