...
%% Check Total Number and Cloud amount
	...
	%                                BB_height_bin(fmonth,ti,BB_bin)=BB_height_bin(fmonth,ti,BB_bin)+1;
	            ...
	            vel_diff_max=vel_diff(BB_loc_max);
	            hi=BB_loc_max;
	            while abs(vel_diff_max)*0.2>abs(vel_diff(hi))&&hi<num_hgt
	                hi=hi+1;
	            end
	            BB_h=height_nc(hi)
	            BB_height(ti)=BB_h
	            hi=BB_loc_max;
	            while abs(vel_diff_max)*0.2>abs(vel_diff(hi))&&hi>0
	                hi=hi-1;
	            end
	            BB_b=height_nc(hi)
	            BB_bottom(ti)=BB_b
	    ...
	    save(matfname,'BB_height','BB_bottom','-append')
